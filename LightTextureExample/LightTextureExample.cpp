...
//<<渲染函数>>
static void display(GLFWwindow* window, unsigned int* shaderProgram_ID, unsigned int* VAO_ID,unsigned int* texture)
{
...
#pragma region Material
	int loctmp = glGetUniformLocation(shaderProgram_ID[0], "material.diffuse");
	glUniform1i(loctmp, 0);
	loctmp = glGetUniformLocation(shaderProgram_ID[0], "material.specular");
	glUniform1i(loctmp, 1);
	loctmp = glGetUniformLocation(shaderProgram_ID[0], "material.shininess");
	glUniform1f(loctmp, 128.f);
...
#pragma region ExplainVertex
	...
	glVertexAttribPointer(textureInShader, textureInShaderDataSize, GL_FLOAT, needNormalize, 8 * sizeof(GL_FLOAT), BUFFER_OFFSET(6 * sizeof(GL_FLOAT)));
	//注意要启用相应的位置
...
#pragma endregion UseProgram
	...
	unsigned int texturesmap[]{ diffuseMap ,specularMap };
...
#pragma region RenderPart
	while (!glfwWindowShouldClose(window)) {
		...
		display(window, shaderProgram_ID, VAO_ID, texturesmap);
		//<<事件触发器>>
		glfwPollEvents();
