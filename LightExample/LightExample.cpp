...
static void display(GLFWwindow* window, unsigned int* shaderProgram_ID, unsigned int* VAO_ID)
{
	...
	//改变光源位置
	LightPositions.x = 1.0f + sin(glfwGetTime()) * 2.0f;
	LightPositions.y = sin(glfwGetTime() / 2.0f) * 1.0f;
...
#pragma region drawCube
	...
	//光源点：漫反射,镜面反射
	loctmp = glGetUniformLocation(shaderProgram_ID[0], "lightPos");
...
#pragma region IDManaged
	...
	//VertexShader
	Shader vertexShader("../shader/lightmode_shader.vert", GL_VERTEX_SHADER);
	Shader vertexLightShader("../shader/lightmode_light_shader.vert", GL_VERTEX_SHADER);


	//FragmentShader
	Shader fragShader("../shader/lightmode_shader.frag", GL_FRAGMENT_SHADER);
	Shader fragLightShader("../shader/lightmode_light_shader.frag", GL_FRAGMENT_SHADER);
