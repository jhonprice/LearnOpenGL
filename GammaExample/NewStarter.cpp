...
#pragma region ShaderProgram
	Shader vertexShader("../shader/gamma_shader.vert", GL_VERTEX_SHADER);
	Shader fragShader("../shader/gamma_shader.frag", GL_FRAGMENT_SHADER);
...
#pragma region SetTexture
	unsigned int planeTexture = loadTexture("../image/wood.png",false);
	unsigned int planeTextureGammaCorrected = loadTexture("../image/wood.png", true);
...
#pragma region RenderFunc
	...
	shaderProgram.setInt("gamma", gammaEnabled);
	// set light uniforms
	glUniform3fv(glGetUniformLocation(shaderProgram.getId(), "lightPositions"), 4, &lightPositions[0][0]);
	glUniform3fv(glGetUniformLocation(shaderProgram.getId(), "lightColors"), 4, &lightColors[0][0]);
	...
	glBindTexture(GL_TEXTURE_2D, gammaEnabled ? planeTextureGammaCorrected : planeTexture);
