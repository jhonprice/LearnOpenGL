...
//<<渲染函数>>
//+=<<EBO方式绘制>>
static void display(GLFWwindow* window, int shaderProgram_ID, int VAO_ID,int EBO_ID)
{
...
#pragma region F_LINE
	int i = 0;
	if (glfwGetKey(window, GLFW_KEY_F) == GLFW_PRESS && i == 0) {
...
#pragma region RenderPart
	while (!glfwWindowShouldClose(window)) {
		...
		//<<渲染函数>>
		//+=<<EBO方式绘制>>
		display(window,shaderProgram_ID,VAO_ID,EBO_ID);
		//<<事件触发器>>
		glfwPollEvents();
